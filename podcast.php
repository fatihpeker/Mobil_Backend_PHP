...
$host = "85.95.237.209";
$user = "iuclabco_bookcast";
$pass = "mobil2020+";
$db = "iuclabco_bookcast";
    ...
    if ($operation_type == "login" && $service_type == "UserList") {
        ...
        if (count($result) == 0) {
            ...
            if (!$process) {
                ...
                $output = createOutput('false', "Kalınan Son Nokta Güncellendi", $data);
                echo json_encode($output);
            }
        ...
        if (count($result) == 0) {
            ...
            $insert_query->bindParam(":pid", $podcast_id, PDO::PARAM_INT);
            $insert_query->bindParam(":email", $email, PDO::PARAM_STR);
            $insert_query->bindParam(":insert_time", $currentDateTime, PDO::PARAM_STR);
            $process = $insert_query->execute();
            ...
            if (!$process) {
                ...
                $output = createOutput('true', 'Yeni Kayıt Alınamadı', []);
                echo json_encode($output);
                return;
            }
            ...
            $output = createOutput('false', "Podcast ve Kullanıcı Eşleştirildi", $data);
            echo json_encode($output);
